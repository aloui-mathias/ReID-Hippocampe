...
def export_tflite(keras_model, im, file, int8, data, ncalib, prefix=colorstr('TensorFlow Lite:')):
    ...
    try:
        ...
        if int8:
            ...
            dataset = LoadImages(check_dataset(
                data)['train'], img_size=imgsz, auto=False)  # representative data
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    assert sum(flags) == len(
        include), f'ERROR: Invalid --include {include}, valid --include arguments are {formats}'
    # export booleans
    ...
    if any((saved_model, pb, tflite, edgetpu, tfjs)):
        ...
        assert not (
            tflite and tfjs), 'TFLite and TF.js models must be exported separately, please pass only one type.'

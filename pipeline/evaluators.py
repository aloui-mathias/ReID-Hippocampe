...
class evaluator:
    ...
    def evalProfils(
        ...
        indiv_y = np.array(list(
            map(str.split, y, ["."] * len(y))))[:, 0]

        for label in tqdm(np.unique(indiv_y)):
            print("Label : ", label)
            indices_left = list(np.where(
                y == ".".join([label, "left"]))[0])
            indices_right = list(np.where(
                y == ".".join([label, "right"]))[0])
